...
in the prescribed time and under specific conditions is called (\AI{B}).
...
\startQ
The {\ftEmp{POSSIBLE}} result of following code is (\AI{C}).
...
\startQ
Please fill in the blank statements of the following binary-search function.
...
bool bin_search(node_t *pNode, int8_t item)
{
	if (pNode == nullptr) {
		...
		if (item == pNode->data) {
			return /BTEX\AI{true}/ETEX;
		} else if( item < pNode->data) {
			return  bin_search(/BTEX\AI{pNode->left}/ETEX, item);
		} else {
			return bin_search(/BTEX\AI{pNode->right}/ETEX, item);
		}
	}
}
\stopC
...
\startQ
The function of following function is: \startAI计算 \ccmm{x} 中比特 1 的个数\stopAI。
\startC
...
\ccmm{strlen(str)} and \ccmm{sizeof(p)} are: (\AI{B}).
\startC
...
How to build the road to make it shortest? ({\ftEmp{hint: shifting}})
...
and explain the purpose of everyone respectively.
\stopQ
\startA
常用的 \ckey{public}、 \ckey{protected}、 \ckey{private}。
还有一个不常用的 \ckey{friend}。
\ckey{public} 即公共的，任 何人都能访问；
\ckey{protected} 即只有自己和子类可以访问；
\ckey{private} 即只有自己才能访问。
\stopA

\startQ
Please define a macro to return the smaller one of two variables.
\stopQ
\startA
\startC
...
\startQ
The output of following program is ({\AI{B}}).
...
on a scale pan (秤盘) to weigh any integral number of pounds
from 1 to 40 inclusive,
if the weights can be placed in either of the scale pans?
({\ftEmp{Using Mathematical Induction to get the recurrence formula}})
\stopQ
\startA
\startformula \startalign
\NC a_{n + 1} \NC = S_n + 1 \NR
