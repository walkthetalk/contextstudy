...
-- 處理中文中的英文斷行，主要用於代碼中的變量名、枚舉名等。
--'a~z'
local function is_lower_letter_node (n)
	if 97 <= n.char and n.char <= 122 then
		return true
	end
	return false
end
...
--'A~Z'
local function is_upper_letter_node (n)
	if 65 <= n.char and n.char <= 90 then
		return true
	end
	return false
end
...
--'A~Z a~z'
local function is_letter_node (n)
	if is_lower_letter_node (n) or is_upper_letter_node (n) then
		return true
	end
	return false
end
...
--'_'
local function is_underline_node (n)
	if n.char == 95 then
		return true
	end
	return false
...
local function is_basic_latin_node (n)
	if 0x20 <= n.char and n.char <= 0x7F then
		return true
	end
	return false
end

local function need_insert_EnInZh_linebreak (n)
	if n.next == nil then
		return false
	end
	if n.id ~= glyph_flag or n.next.id ~= glyph_flag then
		return false
	end
...
local function need_insert_space_between_cjk_and_latin (n)
	if n.id ~= glyph_flag then
		return false
	end

	local nn = n.next
	if nn == nil or nn.id ~= glyph_flag then
		return false
	end

	if is_cjk_ideo(n) and not is_cjk_ideo(nn) and cjk_punc_left[n.char] == nil then
		return true
	end
	if not is_cjk_ideo(n) and is_cjk_ideo(nn) and cjk_punc_right[nn.char] == nil then
		return true
	end
...
local function add_fixed_space (head)
	-- 在中英文间加入空格
	for n in node_traverse_id (glyph_flag, head) do
		if need_insert_space_between_cjk_and_latin (n) then
			local parameters = fonts.hashes.identifiers[font.current()].parameters
			local space = nodes.pool.glue(parameters.space,parameters.space_stretch,parameters.space_shrink)
			node.insert_after(head, n, space)
		end

		--if need_insert_EnInZh_linebreak (n) then
		--	node.insert_after(head, n, nodes.pool.glue (0,0,0))
		--end
	end
end
