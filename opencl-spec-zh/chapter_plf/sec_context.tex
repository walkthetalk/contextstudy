...
\item \carg{private_info} 指向一塊二進制數據，其大小為 \carg{cb}，這塊數據由 OpenCL 實作所返回，可用來記錄一些附加信息來幫助調試錯誤。
\item \carg{user_data} 指向用戶提供的數據。
\stopigBase

如果 \carg{pfn_notify} 是 \cenum{NULL}，則表示不註冊回調函式。
...
%clGetContextInfo
函式 \capi{clGetContextInfo} 可用來查詢\cnglo{context}的相關信息。
\startclc
