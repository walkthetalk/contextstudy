...
\carg{device_type} 是位欄（ bitfield )，用來標識 OpenCL \cnglo{device}的類型。
...
\carg{param_value} 是指針，所指內存用來存儲 \carg{param_name} 所對應的值
...
或者 \cenum{param_value_size} 的值 < \reftab{cldevquery}中返回型別的大小，
