...
\carg{platform} 即 \capi{clGetPlatformIDs} 所返回的\cnglo{platform} ID，或者是 \cmacro{NULL}。
...
\item \cenum{CL_INVALID_VALUE}，如果 \carg{num_entries} 等於零且 \carg{devices} 不是 \cmacro{NULL}，或者 \carg{num_devices} 和 \carg{devices} 都是 \cmacro{NULL}。
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%clGetDeviceInfo
函数 \capi{clGetDeviceInfo} 可用來獲取 OpenCL \cnglo{device}的特定資訊，如\reftab{cldevquery}所示。
\startCLFUNC
...
如果是\cnglo{subdev}，則返回的是\cnglo{subdev}的資訊。
...
或者 \cenum{param_value_size} 的值 < \reftab{cldevquery}中返回型別的大小並且 \cenum{param_value} 不是 \cmacro{NULL}，
