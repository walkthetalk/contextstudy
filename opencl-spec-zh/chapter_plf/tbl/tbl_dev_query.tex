...
\clETD{
  ...
  如果不支持雙精度浮點型別，\cenum{CL_DEVICE_PREFERRED_VECTOR_WIDTH_DOUBLE} 必須返回0。

  如果不支持擴展 \clext{cl_khr_fp16}，\cenum{CL_DEVICE_PREFERRED_VECTOR_WIDTH_HALF} 必須返回0。
}
...
\clETD{
  ...
  如果不支持雙精度浮點型別，\cenum{CL_DEVICE_NATIVE_VECTOR_WIDTH_DOUBLE} 必須返回0。

  如果不支持擴展 \clext{cl_khr_fp16}，\cenum{CL_DEVICE_NATIVE_VECTOR_WIDTH_HALF} 必須返回0。
}
...
\clETD{CL_DEVICE_ADDRESS_BITS}{cl_uint}{
  運算設備的位址空間缺省大小，無符號整形，單位：bit。當前支持 32bit 或 64bit。
}
...
\clETD{CL_DEVICE_MEM_BASE_ADDR_ALIGN}{cl_uint}{
  如果設備類型不是 \cenum{CL_DEVICE_TYPE_CUSTOM}，其最小值是\cnglo{device}所支持的 OpenCL 內建數據型別中最大的那種的大小，單位：bit。
...
\clETD{CL_DEVICE_SINGLE_FP_CONFIG}{cl_device_fp_config}{
  ...
  \item \cenum{CL_FP_ROUND_TO_NEAREST}——支持向最近偶數舍入（ round to nearest even ）。
  \item \cenum{CL_FP_ROUND_TO_ZERO}——支持向零捨入（ round to zero ）。
  ...
  對於類型不是 \cenum{CL_DEVICE_TYPE_CUSTOM} 的\cnglo{device}，其浮點能力至少要是：
...
\clETD{CL_DEVICE_OPENCL_C_VERSION}{char[]}{
  ...
  如果 \cenum{CL_DEVICE_VERSION} 是 OpenCL \scver, 則 \cfmt{major_version.minor_version} 必須是 \scver。
  如果 \cenum{CL_DEVICE_VERSION} 是 OpenCL 1.1, 則 \cfmt{major_version.minor_version} 必須是 1.1。
  如果 \cenum{CL_DEVICE_VERSION} 是 OpenCL 1.0, 則 \cfmt{major_version.minor_version} 可以是 1.0 或 1.1。
}
...
\clETD{CL_DEVICE_PREFERRED_INTEROP_USER_SYNC}{cl_bool}{
  在 OpenCL 和其他 API （如 DirectX ）間共享\cnglo{memobj}時，如果\cnglo{device}的偏好是讓用戶自己負責同步，則其值為 \cenum{CL_TRUE}；
