...
否則，返回下列錯誤之一：
\startigBase
\item \cenum{CL_INVALID_VALUE}，如果 \carg{num_entries} 等於零，且 \carg{platforms} 不是 \cmacro{NULL}；或者 \carg{num_platforms} 和 \carg{platforms} 都是 \cmacro{NULL}。
...
\carg{platform} 即 \capi{clGetPlatformIDs} 所返回的\cnglo{platform} ID，指明要查詢哪個\cnglo{platform}，也可以是 \cmacro{NULL}。
