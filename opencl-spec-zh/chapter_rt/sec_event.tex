...
cl_int clSetEventCallback (cl_event event,
			...
			void *user_data)
\stopclc
可以為\cnglo{cmd}的某個執行狀態註冊一個用戶回調函數。
當 \carg{event} 所關聯的\cnglo{cmd}的執行狀態變成或越過 \carg{command_exec_status} 時，
所註冊的回調函數就會被調用。

調用 \capi{clSetEventCallback} 會將用戶回調函數註冊到 \carg{event} 的回調棧上。
而對於所註冊用戶回調函數的調用順序，則沒有定義。
...
當命令執行成功或者異常終止時都會調用所註冊的回調函數。}。
不保證按執行狀態的變化順序來調用相應的回調函數。
...
\carg{pfn_event_notify} 即\cnglo{app}所註冊的事件回調函數。
此函數可能會被 OpenCL 實現異步調用。
\cnglo{app}要保證此函數是線程安全的。
此函數的參數為：
\startigBase
\item \carg{event} 即此函數所關注的事件。

\item \carg{event_command_exec_status} 即此函數所關注的\cnglo{cmd}執行狀態。
關於\cnglo{cmd}的執行狀態請參見\reftab{clGetEventInfo}。
如果是由於\cnglo{cmd}異常終止而調用的此函數，
...
\item \cenum{CL_INVALID_VALUE}，如果 \carg{pfn_event_notify} 是 \cenum{NULL}
...
並且它所標識的\cnglo{cmd}執行完畢（或終止），
同時沒有正在等待它完成的\cnglo{cmd}，
則此事件就會被刪除。
...
這時要\cnglo{release}它的最後一個\cnglo{refcnt}，開發人員就要小心了。
如果用戶事件作為參數 \carg{event_wait_list} 傳給了 \capi{clEnqueue***}，
或者另一個\cnglo{host}線程正在用 \capi{clWaitForEvents} 等待它，
