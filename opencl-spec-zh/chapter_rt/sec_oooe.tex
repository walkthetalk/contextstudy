\section{內核和內存對象命令的亂序執行}

提交給\cnglo{cmdq}的 OpenCL 函式按被調用的順序入隊，
但執行順序卻是可配置的，既可以\cnglo{inordexec}，也可以\cnglo{outordexec}。
 \capi{clCreateCommandQueue} 的參數 \carg{properties} 可用來指定執行順序。

對於一個\cnglo{cmdq}，如果沒有為其設置屬性 \cenum{CL_QUEUE_OUT_OF_ORDER_EXEC_MODE_ENABLE}，
...
另外，有\cnglo{cmd}可用來等待事件（\capi{clEnqueueMarkerWithWaitList}）
或\cnglo{barrier}（\capi{clEnqueueBarrierWithWaitList}）。
...
可以使用 \capi{clEnqueueNDRangeKernel}、
 \capi{clEnqueueTask} 或 \capi{clEnqueueNativeKernel}
 所返回的\cnglo{evtobj}來插入等待，
