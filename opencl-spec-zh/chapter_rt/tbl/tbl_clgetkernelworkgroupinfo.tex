\startETD[cl_kernel_work_group_info][返回類型]

\clETD{CL_KERNEL_GLOBAL_WORK_SIZE}{size_t[3]}{
利用此機制，
\cnglo{app}可以查詢用來在 \carg{device} 上執行\cnglo{kernel}的全局索引空間的大小
（即 \capi{clEnqueueNDRangeKernel} 的參數 \carg{global_work_size}）。

這要求 \carg{device} 是自定義\cnglo{device}或所執行的\cnglo{kernel}是內建的，
...
它包括實現執行\cnglo{kernel}所需的內存、
\cnglo{kernel}中所聲明的帶有地址限定符 \cqlf{__local} 的變量、
以及為帶有地址限定符 \cqlf{__local} 的指針參數所分配的內存
（其大小由 \capi{clSetKernelArg} 指定）。

對於任一帶有地址限定符 \cqlf{__local} 的指針參數，如果沒有為其指定內存大小，則假定為 0。
}
...
如果給參數 \carg{local_work_size} 所指定的\cnglo{workgrp}大小不是此查詢結果的倍數，
並不會導致函數執行失敗，除非此值超過了\cnglo{device}的數目。
}

\clETD{CL_KERNEL_PRIVATE_MEM_SIZE}{cl_ulong}{
\cnglo{kernel}中每個\cnglo{workitem}至少要使用多少\cnglo{prvmem}。
它包括實現執行\cnglo{kernel}所需的所有\cnglo{prvmem}；
