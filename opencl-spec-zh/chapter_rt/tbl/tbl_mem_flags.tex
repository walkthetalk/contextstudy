...
\clED{CL_MEM_USE_HOST_PTR}{
  ...
  它表明\cnglo{app}想讓 OpenCL 實現使用 \carg{host_ptr} 所引用的內存來存儲\cnglo{memobj}的內容。

  OpenCL 實現可以在\cnglo{device}內存中保存一份 \carg{host_ptr} 所引用的內容用作 cache。
...
\clED{CL_MEM_ALLOC_HOST_PTR}{
    表明\cnglo{app}想讓 OpenCL 實現在\cnglo{host}可以訪問的內存中分配內存。
...
\clED{CL_MEM_COPY_HOST_PTR}{
  僅當 \carg{host_ptr} 不是 \cenum{NULL} 時才有效。
  它表明\cnglo{app}想讓 OpenCL 實現使用 \carg{host_ptr} 所引用的內存來為\cnglo{memobj}分配內存並拷貝數據。
