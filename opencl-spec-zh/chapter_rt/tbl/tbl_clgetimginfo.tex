...
\clETD{CL_IMAGE_ELEMENT_SIZE}{size_t}{
  返回 \carg{image} 中每個元素的大小。一個元素由 $n$ 個通道組成。 $n$ 由 \ctype{cl_image_format} 給出。
}
...
\clETD{CL_IMAGE_SLICE_PITCH}{size_t}{
  對於 3D \cnglo{imgobj}，返回 2D 平面間距；
  對於 1D 或 2D 圖像陣列，返回每個圖像的大小；
  對於 1D 圖像、 1D 圖像 buffer 以及 2D \cnglo{imgobj}，返回 0。
}
...
\clETD{CL_IMAGE_HEIGHT}{size_t}{
  返回圖像高度，單位像素。
  對於 1D 圖像、 1D 圖像 buffer 以及 1D 圖像陣列對象，高度為 0。
}

\clETD{CL_IMAGE_DEPTH}{size_t}{
  返回圖像深度，單位像素。
  對於 1D 圖像、 1D 圖像 buffer、 2D 圖像，以及 1D 或 2D 圖像陣列對象，深度為 0。
}
