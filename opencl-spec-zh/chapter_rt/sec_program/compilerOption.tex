...
就像處理指示（directive） \ccmm{#define} 那樣，在翻譯的第三個階段中會將 \carg{definition} 的內容符號化並處理。
特別是遇到換行符會進行截斷。
\stopclOption
...
而如果沒有使用此選項，則他們的最小精度在\todo{節7.4}中定義。
...
例如，一些 OpenCL \cnglo{device}實作的 \ccmm{mad} 在加 \ccmm{c} 前會對 \ccmm{a * b} 進行截斷。
\stopclOption
...
\item 可能違反 \todo{節 7.4}中所定義的對單雙精度浮點數的數值符合性需求，
還有\todo{節7.5}中的邊界條件下的行為。
\stopigBase
...
這可能違反 \todo{節 7.4}中所定義的對單雙精度浮點數的數值符合性需求，
還有\todo{節7.5}中的邊界條件下的行為。
\stopclOption
...
也可以違反 \todo{節 7.4}中所定義的對單雙精度浮點數的數值符合性需求，
還有\todo{節7.5}中的邊界條件下的行為。
...
此選項允許編譯器將\cnglo{kernel}參數的資訊存儲到可執行\cnglo{program}中。
所存儲的資訊包括參數名、參數型別、所使用的位址限定符以及訪問限定符。
