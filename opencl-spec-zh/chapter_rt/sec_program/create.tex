...
\carg{strings} 是一個字符串陣列，有 \carg{count} 個元素，字符串可以 null 結尾，這些字符串就構成了源代碼。

\carg{lengths} 也是一個陣列，其中的元素代表字符個數（字符串長度）。如果某個元素是零，則相應的字符串以 null 結尾。
...
cl_program clCreateProgramWithBuiltInKernels (cl_context context,
			...
			cl_int *errcode_ret)
\stopclc
會創建一個\cnglo{programobj}，並將內建\cnglo{kernel}的相關信息加載到此對象中。
