...
\placetable[here,force,split][tab:clGetProgramInfo]
...
cl_int clGetProgramBuildInfo (cl_program program,
			...
			size_t *param_value_size_ret)
\stopCLFUNC
會返回\cnglo{programobj}中某個\cnglo{device}所對應的構建資訊。
...
\carg{param_name} 指定要查詢什麼資訊。
對於所支持的資訊類型以及 \carg{param_value} 中返回的資訊，請參見\reftab{clGetProgramBuildInfo}。
...
\placetable[here,force,split][tab:clGetProgramBuildInfo]
