...
如果 \carg{pfn_notify} 不是 \cmacro{NULL}，一旦可以開始編譯， \capi{clCompileProgram} 就會立刻返回，而不必等待編譯完成。
如果\cnglo{context}、所要編譯的\cnglo{program}源碼、\cnglo{device}清單、所輸入的頭檔以及相應的\cnglo{program}，還有構建選項都是有效的，
並且實施編譯所需的\cnglo{host}和\cnglo{device}資源都可用，則就可以開始進行編譯了。
如果 \carg{pfn_notify} 是 \cmacro{NULL}，直到編譯完畢， \capi{clCompileProgram} 才會返回。
對此函式的調用可能是異步的。
\cnglo{app}需要保證此函式是線程安全的。

\carg{user_data} 在調用 \carg{pfn_notify} 時作為參數傳入，可以是 \cmacro{NULL}。
...
\item \cenum{CL_INVALID_VALUE}，如果 \carg{device_list} 是 \cmacro{NULL} 而 \carg{num_devices} 大於零，
或者 \carg{device_list} 不是 \cmacro{NULL} 而 \carg{num_devices} 等於零。

\item \cenum{CL_INVALID_VALUE}，如果 \carg{num_input_headers} 是零，而 \carg{header_include_names} 或 \carg{input_headers} 不是 \cmacro{NULL}；
或者 \carg{num_input_headers} 不是零，而 \carg{header_include_names} 或 \carg{input_headers} 是 \cmacro{NULL}。

\item \cenum{CL_INVALID_VALUE}，如果 \carg{pfn_notify} 是 \cmacro{NULL}，而 \carg{user_data} 不是 \cmacro{NULL}。
