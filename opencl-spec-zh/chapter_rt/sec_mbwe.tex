...
如果 \carg{event_wait_list} 是 \cenum{NULL}，
 \carg{num_events_in_wait_list} 必須是 0。
如果 \carg{event_wait_list} 不是 \cenum{NULL}，
...
或者 \carg{event_wait_list} 不是 \cenum{NULL}，
...
即，所遇之後入隊的\cnglo{cmd}都得等到它完成後才能執行。
此\cnglo{cmd}會返回一個可以等待的事件，
即可以等在這個事件上，來確定 \carg{event_wait_list} 中的事件完成，
或者 \carg{command_queue} 中之前入隊的\cnglo{cmd}完成。
