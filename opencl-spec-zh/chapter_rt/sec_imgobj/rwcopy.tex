...
\topclfunc{clEnqueueReadImage} \topclfunc{clEnqueueWriteImage}
...
\cnglo{app}就可以重新使用 \carg{ptr} 所指向的內存了。
...
\math{origin[2]} 必須是 0， \math{origin[1]} 是圖像在陣列中的索引。
...
如果 \carg{image} 是 2D \cnglo{imgobj}或 1D 圖像陣列對象， \math{region[2]} 必須是 1。
如果 \carg{image} 是 1D \cnglo{imgobj}或 1D 圖像緩衝對象，
...
其值必須大於或等於 \math{\text{元素大小} * width}。
如果 \carg{row_pitch} 或 \carg{input_row_pitch} 是 0，
則會根據 \math{\text{元素大小} * width} 自動計算行間距。
...
否則必須大於或等於 \math{row\_pitch * height}。
如果 \carg{slice_pitch} 或 \carg{input_slice_pitch} 是 0，
則會根據 \math{row\_pitch * height} 自動計算面間距。
...
\item \cenum{CL_INVALID_VALUE}，如果 \math{(origin, region)} 所指定的區域越限，
...
\math{host\_ptr + (origin[2] * image\_slice\_pitch + origin[1] * image\_row\_pitch + origin[0] * bytes per pixel}，
...
如果 \carg{src_image} 是 2D \cnglo{imgobj}， \math{src\_origin[2]} 必須是 0。
如果 \carg{src_image} 是 1D \cnglo{imgobj}，
\math{src\_origin[1]} 和 \math{src\_origin[2]} 都必須是 0。
如果 \carg{src_image} 是 1D 圖像陣列對象， \math{src\_origin[1]} 即圖像索引。
如果 \carg{src_image} 是 2D 圖像陣列對象， \math{src\_origin[2]} 即圖像索引。
...
\math{src\_origin[1]} 和 \math{region[2]} 必須是 1。
...
\carg{event} 會返回一個\cnglo{evtobj}，
用來標識此拷貝\cnglo{cmd}，可用來查詢或等待此\cnglo{cmd}結束。
而如果 \carg{event} 是 \cmacro{NULL}，就沒辦法查詢此\cnglo{cmd}的狀態或等待其完成了。
...
\item \cenum{CL_INVALID_VALUE}，
如果 \math{(src\_origin, region)} 所指定的區域在 \carg{src_image} 中越限，
或者 \math{(dst\_origin, region)} 所指定的區域在 \carg{dst_image} 中越限。
