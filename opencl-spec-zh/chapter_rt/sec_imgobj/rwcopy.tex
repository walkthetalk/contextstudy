...
如果 \carg{image} 是 1D \cnglo{imgobj}或 1D 圖像緩衝對象，
\math{\marg{region}[1]} 和 \math{\marg{region}[2]} 都必須是 0。
...
\math{\marg{host_ptr} + (\marg{origin}[2] * \marg{image_slice_pitch} + \marg{origin}[1] * \marg{image_row_pitch} + \marg{origin}[0] * bytes per pixel}，
