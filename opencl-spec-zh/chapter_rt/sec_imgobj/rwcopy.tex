...
下列函數會將一個用於讀寫\cnglo{imgobj}的\cnglo{cmd}插入隊列，
...
如果 \carg{blocking_write} 是 \cenum{CL_TRUE}， OpenCL 實現會拷貝 \carg{ptr} 所指向的數據，並將寫\cnglo{cmd}插入隊列。
...
\carg{origin} 定義了所要讀寫的區域，對於 1D、 2D 或 3D 圖像而言是 $(x, y, z)$，單位像素；
...
\carg{region} 定義了要讀寫的 1D、 2D 或 3D 區域：$(width, height, depth)$，單位像素。
對於 2D 圖像陣列對象， 即 2D 區域 $(width, height)$，以及圖像個數。
對於 1D 圖像陣列對象， 即 1D 區域 $(width)$，以及圖像個數。
...
\item 由 1D \cnglo{imgobj}拷貝到 2D \cnglo{imgobj} 的掃描線，反之亦可。
\item 由 1D \cnglo{imgobj}拷貝到 3D \cnglo{imgobj} 的 2D 平面的掃描線，反之亦可。
\item 由 1D \cnglo{imgobj}拷貝到 1D 或 2D 圖像陣列對象中某個圖像的掃描線，反之亦可。
...
\carg{dst_origin} 除了對應的是 \carg{dst_image}，其它都跟 \carg{src_origin} 一樣。
