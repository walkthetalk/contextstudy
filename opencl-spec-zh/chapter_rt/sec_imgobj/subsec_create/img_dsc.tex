\subsubsection[tit:imgDsc]{圖像描述符}
...
如果 \carg{host_ptr} 是 \cenum{NULL}，其值必須是 0；
如果 \carg{host_ptr} 不是 \cenum{NULL}，則可以是 0 或者 >= \cvar{image_width} * 元素大小。
如果 \carg{host_ptr} 不是 \cenum{NULL}，並且 \cvar{image_row_pitch} = 0，則用 \cvar{image_width} * 元素大小 代替 \cvar{image_row_pitch}。
如果 \cvar{imag_row_pitch} 不是 0，則必須是圖像元素大小的整數倍。

\cvar{image_slice_pitch} 即 3D 圖像中每個 2D 平面的大小，或者 1D、 2D 圖像陣列中每個圖像的大小。單位字節。
如果 \carg{host_ptr} 是 \cenum{NULL}，其值必須是 0；
如果 \carg{host_ptr} 不是 \cenum{NULL}，
  對於 2D 圖像陣列或 3D 圖像，可以是 0 或者 >= \cvar{image_row_pitch} * \cvar{image_height}；
  對於 1D 圖像陣列，可以是 0 或者 >= \cvar{image_row_pitch}。
如果 \carg{host_ptr} 不是 \cenum{NULL}，並且 \cvar{image_slice_pitch} = 0，
...
如果 \cvar{image_type} 是 \cenum{CL_MEM_OBJECT_IMAGE1D_BUFFER}，則 \cvar{buffer} 引用一個\cnglo{bufobj}，否則 \cvar{buffer} 必須是 \cenum{NULL}。
