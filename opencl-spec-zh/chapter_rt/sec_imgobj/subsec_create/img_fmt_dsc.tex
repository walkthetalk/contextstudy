...
它們都是壓縮過的圖像格式，每個元素的所有通道數據被壓縮到一個無符號短整形或無符號整形中。
...
OpenCL 實現必須保持 \cvar{image_channel_data_type} 中比特數所指定的最小精度。
如果 OpenCL 實現不支持由 \cvar{image_channel_order} 和 \cvar{image_channel_data_type} 所定義的圖像格式，
則 \capi{clCreateImage} 會返回 \cenum{NULL}。
