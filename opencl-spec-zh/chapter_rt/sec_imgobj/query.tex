...
\stopclc
可用來查詢 OpenCL 實現所支持的圖像格式，對於\cnglo{imgobj}，需要指定如下信息：
\startigBase
\item \cnglo{context}
\item 圖像類型 ———— 1D、 2D 或 3D 圖像， 1D 圖像 buffer， 1D 或 2D 圖像陣列
\item \cnglo{imgobj}的分配信息
\stopigBase
...
\carg{flags} 是位域，用來描述所創建的\cnglo{imgobj}的分配和使用信息。參見\reftab{clmapflags}。

\carg{image_type} 即圖像類型，其值可以是： \cenum{CL_MEM_OBJECT_IMAGE1D}、 
\cenum{CL_MEM_OBJECT_IMAGE1D_BUFFER}、 \cenum{CL_MEM_OBJECT_IMAGE2D}、 
\cenum{CL_MEM_OBJECT_IMAGE3D}、 \cenum{CL_MEM_OBJECT_IMAGE1D_ARRAY} 或 
\cenum{CL_MEM_OBJECT_IMAGE2D_ARRAY}。
...
則 OpenCL 實現所賦予下列變量的值必須大於或等於\reftab{cldevquery}中的最小值：
\startigBase
