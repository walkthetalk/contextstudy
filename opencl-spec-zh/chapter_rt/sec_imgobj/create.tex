...
\item \cenum{CL_INVALID_HOST_PTR}，
  如果 \carg{image_desc} 中 \carg{host_ptr} 是 \cmacro{NULL}，且 \carg{flags} 中設置了 \cenum{CL_MEM_USE_HOST_PTR} 或 {CL_MEM_COPY_HOST_PTR}；
  或者 \carg{host_ptr} 不是 \cmacro{NULL}，但 \carg{flags} 中設置了 \cenum{CL_MEM_COPY_HOST_PTR} 或 {CL_MEM_USE_HOST_PTR}。
