...
void * clEnqueueMapImage (cl_command_queue command_queue,
			...
			cl_int *errcode_ret)
\stopclc
所要插入隊列的\cnglo{cmd}會將 \carg{image} 的某個區域映射到\cnglo{host}的位址空間中，並返回新位址。
...
直到映射完成， \capi{clEnqueueMapImage} 才會返回，\cnglo{app}可以用返回的指針存取所映射區域的內容。
...
映射\cnglo{cmd}完成後，\cnglo{app}就可以使用 \capi{clEnqueueMapImage} 所返回的指針存取映射區域的內容了。
...
\carg{image_row_pitch} 即所映射區域的掃描線間距，單位字節，不能是 \cmacro{NULL}。
