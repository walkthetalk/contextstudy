...
對於所有\cnglo{memobj}都適用的信息可以使用 \capi{clGetMemObjectInfo} 來獲取，參見\todo{5.4.5}。

而對於由 \capi{clCreateImage} 創建的\cnglo{imgobj}而言，一些特定信息使用如下函數獲取：
\startclc
...
\carg{param_name} 指明要查詢什麼信息。它所支持的類型以及在 \carg{param_value} 中返回的信息如\reftab{clgetimginfo}所示。

\carg{param_value} 用來存儲查詢結果。如果 \carg{param_value} 是 \cenum{NULL}，則忽略。

\carg{param_value_size}，即 \carg{param_value} 內存塊的大小。
其值必須 >= \reftab{clgetimginfo}中返回類型的大小。
...
或者 \cenum{param_value_size} 的值 < \reftab{clgetimginfo}中返回類型的大小並且 \cenum{param_value} 不是 \cenum{NULL}，
...
\splitfloat{
\placetable[here,force][tab:clgetimginfo]{\getbuffer[tblclgetimginfo]}
}{
