...
\stopCLFUNC
會返回\cnglo{kernelobj}的相關資訊。

\carg{kernel} 即所要查詢的\cnglo{kernel}。

\carg{param_name} 指定要查詢什麼資訊。
對於所支持的資訊類型以及 \carg{param_value} 中返回的資訊，請參見\reftab{clGetKernelInfo}。
...
cl_int clGetKernelWorkGroupInfo (cl_kernel kernel,
			...
			size_t *param_value_size_ret)
\stopCLFUNC
會返回\cnglo{kernelobj}針對某個\cnglo{device}的資訊。
...
\carg{param_name} 指定要查詢什麼資訊。
對於所支持的資訊類型以及 \carg{param_value} 中返回的資訊，請參見\reftab{clGetKernelWorkGroupInfo}。
...
\placetable[here,force,split][tab:clGetKernelWorkGroupInfo]
...
cl_int clGetKernelArgInfo (cl_kernel kernel,
			...
			size_t *param_value_size_ret)
\stopCLFUNC
會返回\cnglo{kernel}參數的相關資訊。
只有滿足下列條件時，\cnglo{kernel}參數資訊才可用：
\startigBase
...
\carg{param_name} 指定要查詢什麼資訊。
對於所支持的資訊類型以及 \carg{param_value} 中返回的資訊，請參見\reftab{clGetKernelArgInfo}。
...
\placetable[here,force,split][tab:clGetKernelArgInfo]
...
\item \cenum{CL_KERNEL_ARG_INFO_NOT_AVAILABLE}，如果沒有可用的參數資訊。
