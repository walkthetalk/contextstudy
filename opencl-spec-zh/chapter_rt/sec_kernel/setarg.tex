...
如果參數是一個\cnglo{bufobj}， \carg{arg_value} 可以是 \cenum{NULL}，
也可以指向一個為 \cenum{NULL} 的值，這時，
如果內核參數聲明時帶有限定符 \cqlf{__global} 或 \cqlf{__constant}，
則使用 \cenum{NULL} 值作為參數值。
而如果參數聲明時所帶限定符為 \cqlf{__local}， \carg{arg_value} 必須是 \cenum{NULL}。
如果參數類型是 \ctype{sampler_t}，則 \carg{arg_value} 必須指向\cnglo{sampler}對象。

如果參數是一個指針，指向的是全局或不變地址空間中的內建標量或矢量類型、或用戶自定義結構體，
則作為參數值的\cnglo{memobj}必須是一個\cnglo{bufobj}（或 \cenum{NULL}）。
...
如果參數類型為 \ctype{image2d_t}，則作為參數值的\cnglo{memobj}必須是 2D \cnglo{imgobj}。
如果參數類型為 \ctype{image3d_t}，則作為參數值的\cnglo{memobj}必須是 3D \cnglo{imgobj}。
如果參數類型為 \ctype{image1d_t}，則作為參數值的\cnglo{memobj}必須是 1D \cnglo{imgobj}。
如果參數類型為 \ctype{image1d_buffer_t}，則作為參數值的\cnglo{memobj}必須是 1D 圖像\cnglo{bufobj}。
如果參數類型為 \ctype{image1d_array_t}，則作為參數值的\cnglo{memobj}必須是 1D 圖像陣列對象。
如果參數類型為 \ctype{image2d_array_t}，則作為參數值的\cnglo{memobj}必須是 2D 圖像陣列對象。

對於其它類型的參數， \carg{arg_value} 必須指向作為參數值的實際數據。
...
如果參數類型是 \ctype{sampler_t}，則 \carg{arg_size} 的值必須是 \ccmm{sizeof(cl_sampler)}。
對於其它類型的參數， \carg{arg_size} 即為參數類型的大小。

注意：

\startbuffer[footnotesetkernelargrefcnt]
實現不能讓 \ctype{cl_kernel} 對象擁有其參數的\cnglo{refcnt}，
...
如果參數類型是\cnglo{memobj}，而 \carg{arg_value} 無效。

\item \cenum{CL_INVALID_SAMPLER}，
如果參數類型是 \ctype{sampler_t}，而 \carg{arg_value} 無效。
