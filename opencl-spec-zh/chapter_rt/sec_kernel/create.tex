...
\carg{kernel_name} 是\cnglo{program}中一個函式的名字，聲明時帶有限定符 \cqlf{__kernel}。
...
\stopCLFUNC
會為 \carg{program} 中的所有\cnglo{kernel}函式創建對應的\cnglo{kernelobj}。
如果 \cqlf{__kernel} 函式的定義在已經具有執行體的那些\cnglo{device}上不完全一樣，
