...
（用來存儲\cnglo{memobj}的內容）可以重新使用或是被釋放時，
...
\capi{clEnqueueWriteBuffer} 或 \capi{clEnqueueWriteBufferRect} 的阻塞式調用

\item 對 \capi{clEnqueueReadImage} 或 \capi{clEnqueueWriteImage} 的阻塞式調用

\item 對 \capi{clEnqueueMapBuffer} 或 \capi{clEnqueueMapImage} 的阻塞式調用

\item 對 \capi{clBuildProgram}、 \capi{clCompileProgram}
或 \capi{clLinkProgram} 的阻塞式調用
\stopigBase
