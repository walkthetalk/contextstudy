...
下列函式會將一個用於讀寫\cnglo{bufobj}的\cnglo{cmd}插入隊列，
...
如果 \carg{blocking_write} 是 \cenum{CL_TRUE}， OpenCL 實作會拷貝 \carg{ptr} 所指向的數據，並將一個寫\cnglo{cmd}入隊。
...
%% clEnqueueReadBuffer / clEnqueueWriteBuffer
下列函式會將一個可以讀寫\cnglo{bufobj}中 2D 或 3D 矩形區域的\cnglo{cmd}插入隊列。
\startCLFUNC
...
\item \cenum{CL_MEM_COPY_OVERLAP}，
  ...
  或者 \carg{src_buffer} 和 \carg{dst_buffer} 是同一\cnglo{bufobj}的不同子\cnglo{bufobj}，且他們重疊。
