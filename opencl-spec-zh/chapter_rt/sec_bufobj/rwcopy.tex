...
\math{\marg{buffer_origin}[2] * \marg{buffer_slice_pitch} + \marg{buffer_origin}[1] * \marg{buffer_row_pitch} + \marg{buffer_origin}[0]}。

\carg{host_origin} 定義了要讀寫的內存區域在 \carg{ptr} 中的偏移量 \math{(x,y,z)}。
對於 2D 矩形區域，\math{z} 的值即 \math{\marg{host_origin}[2]} 應該是 0。
偏移量這樣計算：

\math{\marg{host_origin}[2] * \marg{host_slice_pitch} + \marg{host_origin}[1] * \marg{host_row_pitch} + \marg{host_origin}[0]}。
...
如果 \math{(\marg{buffer_origin}, \marg{region}, \marg{buffer_row_pitch}, \marg{buffer_slice_pitch})} 所指定的區域越限。
