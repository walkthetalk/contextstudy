...
\placetable[here,force][tab:clmapflags]
{所支持的 \ctype{cl_map_flags}}
{\input{chapter_rt/tbl/tbl_clmapflags.tex}}

如果執行成功， \capi{clEnqueueMapBuffer} 會返回映射區域的指針，
...
所返回的指針僅可用於訪問所映射的區域 \math{(\marg{offset}, \marg{size})}，雖然實作可能映射更大的區域。
