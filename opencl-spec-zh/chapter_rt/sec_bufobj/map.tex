...
void * clEnqueueMapBuffer (cl_command_queue command_queue,
			...
			cl_int *errcode_ret)
\stopCLFUNC
所要插入隊列的\cnglo{cmd}會將 \carg{buffer} 的某個區域映射到\cnglo{host}的位址空間中，並返回新位址。
...
直到映射完成， \capi{clEnqueueMapBuffer} 才會返回，\cnglo{app}可以用返回的指針存取所映射區域的內容。
...
映射\cnglo{cmd}完成後，\cnglo{app}就可以使用 \capi{clEnqueueMapBuffer} 所返回的指針存取映射區域的內容了。

\carg{map_flags} 是位域，參見\reftab{clmapflags}。

\carg{buffer} 是一個\cnglo{bufobj}，他必須與 \carg{command_queue} 位於同一 OpenCL \cnglo{context}中。
...
所返回的指針僅可用於存取所映射的區域 $(offset,size)$，雖然實作可能映射更大的區域。
對於存取此區域外的內容，其結果未定義。
...
\item \capi{clEnqueueMapBuffer} 所返回的指針得自創建\cnglo{bufobj}時所用的 \carg{host_ptr}。
\stopigBase
