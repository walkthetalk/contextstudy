...
\placetable[here,force,split][tab:clmemflags]
{\carg{cl_mem_flags}的值的清单}
...
\item \cenum{CL_INVLAID_HOST_PTR}，
  如果 \carg{host_ptr} 是 \cmacro{NULL}，并且 \carg{flags} 中设置了 \cenum{CL_MEM_USE_HOST_PTR} 或 \cenum{CL_MEM_COPY_HOST_PTR}；
  或者 \carg{host_ptr} 不是 \cmacro{NULL}，但是 \carg{flags} 中沒有設置 \cenum{CL_MEM_USE_HOST_PTR} 或 \cenum{CL_MEM_COPY_HOST_PTR}。
...
\carg{flags} 是位域，用來表明如何分配以及怎樣使用\cnglo{memobj}，參見\reftab{clmemflags}。
  ...
  即使 \carg{buffer} 的內存存取限定符中有 \cenum{CL_MEM_COPY_HOST_PTR}，也並不意味着創建 sub-buffer 時會有額外的拷貝。
