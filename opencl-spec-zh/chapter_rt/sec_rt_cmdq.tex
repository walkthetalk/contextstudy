...
当 \carg{command_queue} 的\cnglo{refcnt}將為 0，並且其中的所有\cnglo{cmd}全部執行完畢
...
其值必須 >= \reftab{cmdqinfo}中返回類型的大小。如果是 \cenum{NULL}，則忽略。

\carg{param_value_size_ret} 會返回 \carg{param_value} 中查詢結果的實際大小。如果是 \cenum{NULL}，則忽略。
