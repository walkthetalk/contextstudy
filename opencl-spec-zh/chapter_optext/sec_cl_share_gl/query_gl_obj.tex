...
\stopigBase
而如果 \carg{gl_object_type} 是 \cenum{NULL}，則將其忽略。

\carg{gl_object_name} 會返回用來創建 \carg{memobj} 的 GL 對象的名字。
如果 \carg{gl_object_name} 是 \cenum{NULL}，則將其忽略。
...
如果 \carg{param_value} 是 \cenum{NULL}，則將其忽略。
...
如果 \carg{param_value_size_ret} 是 \cenum{NULL}，則將其忽略。
...
或者 \carg{param_value} 和 \carg{param_value_size_ret} 都是 \cenum{NULL}。
