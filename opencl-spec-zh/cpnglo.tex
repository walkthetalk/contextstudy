...
\clglo{app}{應用}{Application}
運行在\refglo{host}和 OpenCL \refglo{device}上的程序。

\clglo{beac}{阻塞的入隊 API 調用}{Blocking Enqueue API calls}
\clglo{nbeac}{非阻塞的入隊 API 調用}{Non-Blocking Enqueue API calls}
...
\item \refglo{cmdqbarrier}本身是一個\refglo{cmd}，
 OpenCL API 提供一個函數可以將其插入隊列。
此命令可以保證在此之前入隊的所有\cnglo{cmd}都執行完畢後，
在此之後入隊的\cnglo{cmd}才能開始執行。
% placeholder B
\item OpenCL C 編程語言內建有一個\refglo{workgrpbarrier}函數。
運行在\refglo{device}上的\refglo{kernel}可以利用此函數在
...
則它所包含的所有\refglo{workitem}都必須先執行此\refglo{barrier}。
\stopigBase
\stopclglo

\startclglo{bufobj}{緩衝對象}{Buffer Object}
存儲線性字節序列的\refglo{memobj}。
\refglo{kernel}在\refglo{device}上運行時，可以通過一個指針來訪問它。
\refglo{host}可以調用 OpenCL API 來操控\refglo{bufobj}。
\refglo{bufobj}封裝了以下信息：
\startigBase
\item 字節數；
...
\clglo{bikernel}{內建內核}{Built-in Kernel}
...
\clglo{cmdqbarrier}{命令隊列屏障}{Command-queue Barrier}
參見\refglo{barrier}。

\clglo{computedevmem}{計算設備內存}{Compute Device Memory}
附着到計算設備上的一塊或多塊內存。

\clglo{computeunit}{計算器件}{Compute Unit}
...
\clglo{dppm}{數據並行編程模型}{Data Parallel Programming Model}
...
\clglo{refcnt}{引用計數}{Reference Count}
...
\clglo{relaxconsist}{放寬的一致性}{Relaxed Consistency}
一種內存一致性模型。
對同一塊內存而言，不同的\refglo{workitem}或\refglo{cmd}所看到的內容可能不同，
當然，\refglo{barrier}和其它顯式同步點除外。

\clglo{res}{資源}{Resource}
OpenCL 所定義的一類\refglo{obj}。
\refglo{res}的一個實例就是一個\refglo{obj}。
...
\clglo{retain}{保留}{Retain}
\clglo{release}{釋放}{Release}
會使 OpenCL \refglo{obj}的\refglo{refcnt}增一（ retain）或減一（ release）。
這是一個記賬功能，
可以保證在使用某個\refglo{obj}的所有實例都結束之前，系統不會移除此\refglo{obj}。
參見\refglo{refcnt}。

\clglo{rootdev}{根設備}{Root Device}
一種 OpenCL \refglo{device}，還未劃分。
請參考\refglo{device}、\refglo{pardev}和\refglo{subdev}。

\clglo{sampler}{採樣器}{Sampler}
...
\clglo{tppm}{任務並行編程模型}{Task Parallel Programming Model}
一種編程模型，其中用多個並發的任務來表示計算，
一個任務就是在\refglo{workgrp}（大小是1）中執行的一個\refglo{kernel}。
這些並發的任務可以運行不同的\refglo{kernel}。

\clglo{thsafe}{線程安全}{Thread-safe}
當一個 OpenCL API 被多個\refglo{host}線程同時調用時，
如果 OpenCL 所管理的內部狀態始終保持一致，就認為它是\refglo{thsafe}的。
如果一个 OpenCL API 是\refglo{thsafe}的，
那麼就允許\refglo{app}在多個\refglo{host}線程中同時調用它，
而不必在這些線程間實施互斥，即它們也是重入安全的。

\clglo{undef}{未定義}{Undefined}
調用 OpenCL API、\refglo{kernel}中使用內建函數或者執行\refglo{kernel}時，
如果 OpenCL 沒有對其行為顯式定義，那麼這種行為就是\refglo{undef}的。
至於在 OpenCL 中碰到這種情況時會發生什麼，不要求實現明確指出。

\clglo{workgrp}{作業組}{Work-group}
...
\clglo{workgrpbarrier}{作業组屏障}{Work-group Barrier}
參見\refglo{barrier}。

\clglo{workitem}{作業項}{Work-item}
