...
\refglo{kernel}在\refglo{device}上運行時，可以通過指針來訪問他。
...
\refglo{computeunit}也可能包含專用的材質濾波器件，此器件可以被他的\refglo{prcele}訪問。
\stopclglo

\startclglo[concurrency][並發][Concurrency]
系統的一個屬性，這樣的系統中，多個任務可以同時保持活躍並取得進展。
運行\refglo{program}時要想並發執行，程序員必須找出問題中可以並發的部分，
...
\refglo{context}包括一組\refglo{device}、這些\refglo{device}可以訪問的內存、
...
一塊\refglo{memregion}，\refglo{context}中的所有\refglo{workitem}均可訪問。
\refglo{host}可以使用\refglo{cmd}來訪問他（如讀、寫以及映射）。
\stopclglo
...
\startclglo[hostptr][主機指針][Host pointer]
...
只能通過讀寫函式訪問其中的圖像數據。讀取函式需要使用\refglo{sampler}。
...
只有這個\refglo{workgrp}中的\refglo{workitem}才能訪問。
\stopclglo
...
\item 最近成功構建的程序執行體，用來運行此\refglo{program}的\refglo{device}清單，
所用的構建選項以及構建日誌。
...
統一建模語言（http://www.uml.org/）是 Object Management Group （OMG）的商標。}
