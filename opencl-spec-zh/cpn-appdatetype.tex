...
\section[appendix:appDataTypeAlignment]{應用數據型別的齊位}

用戶要負責確保傳入和傳出 OpenCL 緩衝的數據相對於緩衝的起始位址原生對齊，
遵守\refsec{alignmentOfTypes}中的要求。
其中隱含着，
用 \cenum{CL_MEM_USE_HOST_PTR} 創建 OpenCL 緩衝時所提供的\cnglo{host}內存指針
...
必須要能用 \ccmm{cl_type} 聯合體中欄位的名字直接訪問才行（\refappendixsec{appendixVecCom}）。
...
可以使用 \ccmm{.sN}、 \ccmm{.sn} 或 \ccmm{.xyzw} 來訪問矢量數據型別的組件，
跟 OpenCL 語言中所用類似。
\ccmm{.xyzw} 僅可訪問前四個組件。
...
可以使用 \ccmm{.hi} 和 \ccmm{.lo} 來訪問矢量數據型別的組件，
跟所支持的語言型別類似。
...
用 \ccmm{__cl_float4} 可能可以直接訪問內建的 \ccmm{__m128} 或矢量浮點型別，
而 \ccmm{cl_float4} 則會被視為聯合體。

另外，為方便訪問，上述\cnglo{app}數據型別可能含有原生矢量數據型別的成員。
用子矢量符號 \ccmm{.vN} 訪問原生組件，其中 \ccmm{N} 為子矢量中的元素編號。
