...
\setuptolerance[horizontal,
  stretch, %stretch space verystrict strict tolerant verytolerant
]
...
\setuptables[
  ...
  %bodyfont	5pt ... 12pt small big
  %HL	number small medium big none
  %VL	number small medium big none
  %depth	number strut
  %height	number strut
  %rulethickness	dimension
  %rulecolor	name
  %align	flushright middle flushleft normal
  %commands	command
  %align	left middle right normal
  %background	screen color none
...
\define\scver{1.1}
\define\scclver{\scopencl\space 1.1}
\define\schostfailres{在为\cnglo{host}上的\scopencl实现分配资源时失败}
\define\scdevfailres{在为\cnglo{device}上的\scopencl实现分配资源时失败}
\define\scdenorm{去规格化}
\define\scinf{INF}
\define\scqnan{QNaN}
\define\scfma{FMA}
\define\scnfma{积和熔加运算}
...
\define\switchtobodyfontsmall{\switchtobodyfont[8pt]}
\setupTABLE[%
    ...
    %
    %color=,
    %style=\switchtobodyfontsmall,
    %headstyle=\switchtobodyfontsmall,
    %headcolor=,
    %aligncharacter=\v!no,
    %alignmentcharacter={,},
    %option=, % \v!stretch
...
\define[2]\cltable{
#1{\bTABLE\bTR\bTD\eTD\eTR\eTABLE}
\startlocalfootnotes
%\placelegend%大括号一定要另起一行，否则会右移，但是会导致TABLE横排，因此注掉
{\bTABLE[split=repeat, option=stretch, style=\rmx, headstyle=\rmx\bf] #2 \eTABLE}
\placelocalfootnotes[style=\rmx,before=,after=,]
\stoplocalfootnotes
}

\define[3]\clenumretdesc{
\bTR \bTC \cenum{#1} \eTC
  \bTC
    \ctype{#2}
  \eTC
