...
裝載函數可從內存中讀取標量或矢量 \cldts{half} 值 並將其轉換成 \cldts{float} 值。
...
對於具備 3 個構件的矢量數據型別，其大小為 \math{4 \times sizeof(component)}。
...
OpenCL 編譯器負責數據項的對齊。
如果 \cqlf{__kernel} 函數的參數聲明為某種數據型別的指針，
...
對於具有 1 到 4 個構件的矢量數據型別，其構件可以用 \ccmm{<vector_data_type>.xyzw} 來尋址。
...
對於 3 構件的矢量型別應用後綴 \ccmm{.lo} （或 \ccmm{.even}）
和 \ccmm{.hi} （或 \ccmm{.odd}）時，
會將 3 構件矢量型別當成 4 構件矢量型別使用，且構件 \ccmm{w} 未定義。
下面給出了一些例子：
\startclc
float8	vf;
float4	odd = vf.odd;
