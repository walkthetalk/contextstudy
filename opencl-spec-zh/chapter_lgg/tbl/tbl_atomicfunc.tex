...
計算 \math{(old+val)} 並將結果存儲到 \carg{p} 所指位置中。
此函式返回 \math{old}。
\stopbuffer
...
讀取 \carg{p} 所指向的 32 位值（記為 \math{old}）。
計算 \math{\text{\capi{min}}(old,val)} 並將結果存儲到 \carg{p} 所指位置中。
此函式返回 \math{old}。
\stopbuffer
...
讀取 \carg{p} 所指向的 32 位值（記為 \math{old}）。
計算 \math{\text{\capi{max}}(old,val)} 並將結果存儲到 \carg{p} 所指位置中。
此函式返回 \math{old}。
\stopbuffer
...
讀取 \carg{p} 所指向的 32 位值（記為 \math{old}）。
計算 \math{(old \text{\ccmm{&}} val)} 並將結果存儲到 \carg{p} 所指位置中。
此函式返回 \math{old}。
\stopbuffer
