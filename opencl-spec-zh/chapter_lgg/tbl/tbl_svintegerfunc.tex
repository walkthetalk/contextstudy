...
返回 \math{(x+y)>>1}，中間的加法不會有模上溢\footnote{
...
返回 \math{\text{\capi{min}}(\text{\capi{max}}(x,minval),maxval)}。
...
返回 \carg{x} 前端有多少位連續的 0，從最高有效位開始。
\stopbuffer
...
計算 \math{x \times y}，並返回乘積的高位半部（high half）。
\stopbuffer
...
將 \math{v} 中的每個元素都循環左移，其位數就是 \math{i} 中對應元素的值
...
% upsample
\startbuffer[funcproto:upsample]
...
\stopbuffer
\startbuffer[funcdesc:upsample]
\math{result[i] = ((short)hi[i] << 8) | lo[i]}\par
\math{result[i] = ((ushort)hi[i] << 8) | lo[i]}\par

\math{result[i] = ((int)hi[i] << 16) | lo[i]}\par
\math{result[i] = ((uint)hi[i] << 16) | lo[i]}\par
