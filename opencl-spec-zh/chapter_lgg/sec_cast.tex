...
運算元和結果的型別可能相同，這種情況下不會進行轉換，算式的型別和值都不變。
...
而且位元的順序也可由實作自由選擇。
不能使用運算子 \clapis{as_type} 和 \clapiv{as_type} 將數據重釋為另一種字節數不同的型別。
...
許多期望運算元為算術類型的運算子所引起轉換並得到結果的方式都類似。
