...
這個大小是由運算元的型別所決定的。
結果的型別為 \ctype{size_t}。
如果運算元的型別為變長數組\footnote{OpenCL 1.1 不支持變長數組，參見\todo{節 6.9.d}。}，
則會對運算元進行求值；
否則，不求值，並且結果是整形常數。
...
無效值指的是運算元為 null 指針、地址沒有根據對象的型別正確對齊、所指對象的生命周期已經結束。
...
要想進行其他轉換，必須顯式指定。
左值必須是可寫的。
如果變量是內建型別、整個結構體或數組、結構體欄位、
用欄位選擇器（\ccmm{.}）對構件進行選擇或重排（欄位互不重複）所形成的左值、
括號中的左值、或者用數組下標運算子（\ccmm{[]}）提領出的左值，
那麼這個變量就是左值。
