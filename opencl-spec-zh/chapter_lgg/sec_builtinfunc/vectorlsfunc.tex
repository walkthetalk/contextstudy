...
泛型 \cldts{gentype} 表示內建數據型別
 \cldts{char}、 \cldts{uchar}、 \cldts{short}、 \cldts{ushort}、
 \cldts{int}、 \cldts{uint}、 \cldts{long}、 \cldts{ulong}、
 \cldts{float} 或 \cldts{double}。
泛型 \cldtv{gentype} 表示具有 n 個 \cldts{gentype} 元素的矢量。
我們用 \cldtv{half} 表示具有 n 個 \cldts{half} 元素的矢量\footnote{
僅在擴展 \clext{cl_khr_fp16} 中才定義有 \cldtv{half}
（參見 OpenCL 1.2 擴展規範的節 9.5）。}。
函式名中也有後綴 \cldtvfix{n} （即 \clapiv{vload}、 \clapiv{vstore} 等），
