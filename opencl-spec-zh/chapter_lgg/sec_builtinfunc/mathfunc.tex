...
在\reftab{hnMathFunc}中，通用型別名 \ctype{gentype} 表示函式引數的型別可以是
 \ctype{float}、 \ctype{float2}、 \ctype{float3}、 \ctype{float4}、
 \ctype{float8} 或 \ctype{float16}。
...
如果定義了此巨集，則表明對 \ctype{float} 運算元的乘、加運算，
...
如果定義了此巨集，則表明對 \ctype{double} 運算元的乘、加運算，
函式 \capi{fma} 一般跟直接編碼一樣快，或者更快。
\stopigBase
