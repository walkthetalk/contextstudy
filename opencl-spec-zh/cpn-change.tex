...
\startigBase
\item \cenum{CL_DEVICE_NATIVE_VECTOR_WIDTH_{CHAR | SHORT | INT | LONG | FLOAT | DOUBLE | HALF}}
...
\item \capi{clEnqueueReadBufferRect}、 \capi{clEnqueueWriteBufferRect} 和
\capi{clEnqueueCopyBufferRect} 可以分別讀、寫、拷貝\cnglo{bufobj}的某個矩形區域。
...
\item \clext{cl_khr_global_int32_base_atomics}、 \clext{cl_khr_global_int32_extended_atomics}、
\clext{cl_khr_local_int32_base_atomics} 和 \clext{cl_khr_local_int32_extended_atomics} 都移入了核心規範。
內建原子函式的名字前綴由 \capi{atom_} 變成了 \capi{atomic_}。
...
\item 擴充了 \carg{cl_mem_flags}，以描述\cnglo{host}如何訪問 \ctype{cl_mem} 對象中的數據。
