...
如果 \cenum{CL_DEVICE_IMAGE3D_MAX_WIDTH}、 \cenum{CL_DEVICE_IMAGE3D_MAX_HEIGHT}
和 \cenum{CL_DEVICE_IMAGE3D_MAX_DEPTH} 是零，
則嵌入式規格中調用 \capi{clCreateImage} 創建 3D 圖像將失敗。
\capi{clCreateImage} 的引數 \carg{errcode_ret} 將返回 \cenum{CL_INVALID_OPERATION}。
\cnglo{kernel}中聲明型別為 \ctype{image3d_t} 的引數將會導致編譯錯誤。

如果 \cenum{CL_DEVICE_IMAGE3D_MAX_WIDTH}、 \cenum{CL_DEVICE_IMAGE3D_MAX_HEIGHT}
和 \cenum{CL_DEVICE_IMAGE3D_MAX_DEPTH} 大於零，
則表明嵌入式規格的實作是支持 3D 圖像的。
...
\cenum{CLK_ADDRESS_NONE}、 \cenum{CLK_ADDRESS_MIRRORED_REPEAT}、
\cenum{CLK_ADDRESS_REPEAT}、 \cenum{CLK_ADDRESS_CLAMP_TO_EDGE}、
\cenum{CLK_ADDRESS_CLAMP}。
\stopitem
...
則結果的值\cnglo{impdef}。
同樣，如果某個算元是 NaN，
則單精度比較算子（<、 >、 <=、 >=、 =、 !=）所返回的值也\cnglo{impdef}。

所有情況下，轉換（\refsec{conversionCast}和\refsec{vectorLsFunc}）
都要像 \cenum{FULL_PROFILE} 一樣正確捨入，包括那些會小號或產生 INF 或 NaN 的轉換。
...
如果減法和乘法缺省為向零捨入的模式，則對於向零捨入，
\capi{fract}、 \capi{fma} 和 \capi{fdim} 應當產生的結果應當正確捨入了。
...
在嵌入式規格中，\refsec{normIntToFloat}中的異常情況以及下列所給處的異常情況都有效。
\startclc
For CL_UNORM_INT8
...
則實作指派給
\cenum{CL_DEVICE_MAX_READ_IMAGE_ARGS}、 \cenum{CL_DEVICE_MAX_WRITE_IMAGE_ARGS}、
\cenum{CL_DEVICE_IMAGE2D_MAX_WIDTH}、 \cenum{CL_DEVICE_IMAGE2D_MAX_HEIGHT}、
\cenum{CL_DEVICE_IMAGE3D_MAX_WIDTH}、 \cenum{CL_DEVICE_IMAGE3D_MAX_HEIGHT}、
\cenum{CL_DEVICE_IMAGE3D_MAX_DEPTH}、 \cenum{CL_DEVICE_MAX_SAMPLERS}
的值必須大於或等於上表中給出的最小值。
